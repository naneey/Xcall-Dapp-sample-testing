...
goloop rpc call --to $XCALL \
      ...
      --param _net='0x61.bsc' \
      --param _rollback=0x1 | jq -r . | tee tx/xcallFee

echo ">>>>> send message from dapp"
_value='17591602201192300288'
message='0x6d6573736167652c64656c697665726564'
goloop rpc sendtx call --to $DAPP_ADDRESS  \
    ...
    --param _data=$message \
    --param readDummyData=0x0 \
    --param multipleMessage=0x1 | jq -r . | tee tx/sendDAPPMessage.icon
